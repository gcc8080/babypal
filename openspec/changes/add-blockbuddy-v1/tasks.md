...
>
> **音效补验（2026-07-26 二轮）**：`tool/gen_sfx.dart` 程序化合成 6 个音效后接入沙盒。真机确认 soloud 初始化成功、输出流 `state:started`（`dumpsys audio` 中 OpenSL ES AudioPlayer 挂在本应用 pid 上）、连续快速点击无异常日志。**音效是否好听、有无丢音需人耳判断**。
>
