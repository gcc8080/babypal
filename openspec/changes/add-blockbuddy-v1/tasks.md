...
>
> **换十与庆祝的播报目前借用数词音频**（`zh/en.number.*`）：`manifest.json` 里只有 0–100 的数词，没有「加 / 等于 / 可以分成」这类短语。**遗留项 → 4.4 需要算式短语时一并扩 `tool/gen_audio.dart`**，届时把这两处换成完整播报。
>
> 33 个新用例（21 纯逻辑 + 12 组件），全量 154 通过，`flutter analyze` 干净。**真机手感未验**——MI 8 SE 当前未连接。
